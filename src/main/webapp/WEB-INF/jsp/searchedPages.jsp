...
<c:if test="${!page.pageItems.isEmpty()}">
    <div class="right-block">
        ...
        %>
        <a href="?query=<%=request.getParameter("query")%>&sortType=relevant&pages=1">Sort by relevant</a> | Sort by alphabet
        <%
        } else {
        %>
        Sort by relevant | <a href="?query=<%=request.getParameter("query")%>&sortType=alphabet&pages=1">Sort by alphabet</a>
        <%
            }
        %>
    </div>
...
<div class="pagination">
    ...
    <c:if test="${currentPage != 1}">
        <a href="?query=${param.query}&sortType=${param.sortType}&pages=${currentPage - 1}">&laquo;</a>
    </c:if>

    <c:forEach begin="${startPage}" end="${endPage}" step="1" varStatus="status">
        <c:choose>
            <c:when test="${param.pages eq status.index}">
                <a class="active" href="?query=${param.query}&sortType=${param.sortType}&pages=${status.index}">${status.index}</a>
            </c:when>
            <c:otherwise>
                <a href="?query=${param.query}&sortType=${param.sortType}&pages=${status.index}">${status.index}</a>
            </c:otherwise>
        </c:choose>
    </c:forEach>

    <c:if test="${currentPage lt lastPage}">
        <a href="?query=${param.query}&sortType=${param.sortType}&pages=${currentPage + 1}">&raquo;</a>
    </c:if>
</div>
</body>
</html>
